...
contract GameLogic{
  ...
  address public erc1155GameAddress;
  ...
  error Unauthorized(address caller);
   ...
   constructor(address signer_,address erc1155GameAddress_ , address roleAddress_,address gamePoolAddress_){
      owner = msg.sender;
   ...
   */
   function mergeToBd(uint256 newId,bytes memory signature,uint256  currentTimeStamp) external nonReentrant {
   ...
   function createRole(address player,uint256 tokenId) 
    ...
    function createGamePrope(address player,uint256 tokenId,uint256 amount,bytes memory data)
        ...
        if(msg.sender != owner){
         ...
         IErc1155Asset(erc1155GameAddress).mint(player, tokenId, amount, data);
    ...
    function batchCreateGamePrope(address player,uint256 [] memory tokenIds,uint256[] memory amounts,uint64 length,bytes memory data)
     ...
     returns(uint256[] memory tokendIds) {
      ...
      for(uint i =0;i< length;i++){
       ...
       IErc1155Asset(erc1155GameAddress).mintBatch(player, tokenIds, amounts, data);
        return tokenIds;
    }
